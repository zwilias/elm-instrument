...
transform modul =
  let
    (comments, modulImports) = M.imports modul :: (AST.Comments, Map.Map [AST.UppercaseIdentifier] (AST.Comments, M.ImportMethod))
    ...
    updatedImports :: Map.Map [AST.UppercaseIdentifier] ([AST.Comment], M.ImportMethod)
    updatedImports =
      Map.insert coverageIdentifier ([], coverageImportMethod) modulImports

    moduleName_ :: [AST.UppercaseIdentifier]
    (AST.Commented _ moduleName_ _) =
      M.name $ M.header modul
  ...
  in
    modul
      { M.imports = (comments, updatedImports)
...
annotate moduleName declaration' =
  case declaration' of
    Declaration.Decl (A region declaration_) ->
      do
...
toVarRef region ann =
  let
    identifier name =
      A region $ Expression.VarExpr $ Var.VarRef [ AST.UppercaseIdentifier "Coverage" ] (AST.LowercaseIdentifier name)
  in
  case ann of
    Expression ->
    ...
    CaseBranch ->
      identifier "caseBranch"

    IfElseBranch ->
      identifier "ifElseBranch"

plain :: a -> AST.PreCommented a
plain a = ( [], a)


annotation :: Annotation -> String -> Expression.Expr -> State AnnotationStore Expression.Expr
annotation annotationType moduleName expression@(A region _) =
  do
      store <- State.get
      let (count, store') = registerA annotationType region store
      State.put store'

      return $ A region $ Expression.App (toVarRef region annotationType)
...
annotateDeclaration :: String -> Declaration.Declaration -> State AnnotationStore Declaration.Declaration
annotateDeclaration moduleName declaration =
  case declaration of
    Declaration.Definition pat pats comments body ->
        do
...
annotateCommentedExpression :: String -> AST.PreCommented Expression.Expr -> State AnnotationStore (AST.PreCommented Expression.Expr)
annotateCommentedExpression moduleName (comments, expr) =
    do
        expr' <- annotateExpression moduleName expr
        return (comments, expr')

annotateExpression :: String -> Expression.Expr -> State AnnotationStore Expression.Expr
annotateExpression moduleName expression@(A region expr) =
  case expr of
    Expression.App inner parts faApp ->
        do
            parts' <- mapM (annotateCommentedExpression moduleName) parts
            inner' <- annotateExpression moduleName inner
            annotation Expression moduleName
                $ A region
                $ Expression.App inner' parts' faApp
    ...
    Expression.Let letDecl comments body ->
        do
    ...
    Expression.Parens (AST.Commented pre bodyContent post) ->
        do
            bodyContent' <- annotateExpression moduleName bodyContent
            return
                $ A region
                $ Expression.Parens
                $ AST.Commented pre bodyContent' post

    Expression.Binops first content bool ->
        do
            first' <- annotateExpression moduleName first
            content' <- mapM (\(comments, ref, moreComments, e) ->
                do
    ...
    Expression.ExplicitList terms trailingComments forceMultiline ->
        do
            terms' <- mapM (\(comments, (preComments, (e, eol))) ->
                do
                    e' <- annotateExpression moduleName e
    ...
    Expression.Case (AST.Commented preOf of_ postOf, flag) patterns ->
        do
            ...
            patterns' <- mapM (\(pat, (comments, body)) ->
                do
                    body' <- annotateExpression moduleName body
            ...
            let caseOf = A region $ Expression.Case (AST.Commented preOf of_' postOf, flag) patterns'
            annotation Expression moduleName caseOf

    Expression.If ifClause otherClauses (comments, elseExpr) ->
        let
            annotateIfClause :: Expression.IfClause -> State AnnotationStore Expression.IfClause
            annotateIfClause
        ...
        do
            ...
            otherClauses' <- mapM (\(clauseComments, clause) ->
                do
                    clause' <- annotateIfClause clause
                    return (clauseComments, clause')
                ) otherClauses
            elseExpr' <- annotateExpression moduleName elseExpr
...
initCoverageDeclaration moduleName store =
    Declaration.Decl
        ...
        $ Declaration.Definition
            ( A emptyRegion
                $ Pattern.VarPattern
                $ AST.LowercaseIdentifier "initCoverage"
            )
            []
            []
...
annotationTypeStoreToPair name store =
    AST.Pair
...
regionsToIdentifiers regions =
    let
        identifiers =  List.foldl' (\acc region ->
                regionToRecord region : acc
            ) [] regions
    in
    A emptyRegion
        $ Expression.ExplicitList
            (toSequence identifiers)
            []
            (AST.ForceMultiline True)
...
regionToRecord Region{start, end} =
    let
        ...
        ctor =
            A emptyRegion
                $ Expression.VarExpr
                $ Var.TagRef [AST.UppercaseIdentifier "Coverage"] (AST.UppercaseIdentifier "Identifier")
    in
    A emptyRegion
        $ Expression.App ctor
...
positionToTuple Position{ line, column } =
    A emptyRegion
        $ Expression.Tuple
            [ AST.Commented [] (A emptyRegion (Expression.Literal $ AST.IntNum (fromIntegral line) AST.DecimalInt)) []
            , AST.Commented [] (A emptyRegion (Expression.Literal $ AST.IntNum (fromIntegral column) AST.DecimalInt)) []
            ]
            False
...
registerA ann =
    case ann of
        ...
        IfElseBranch ->
            registerIfElseBranch


registerExpression :: Region -> AnnotationStore -> (Int64, AnnotationStore)
registerExpression region store =
  let
    ( i, newExpressions ) = register region $ expressions store
  in
  ( i
  , store { expressions = newExpressions }
  )
...
registerCaseBranch region store =
  let
    ( i, newBranches ) = register region $ caseBranches store
  in
  ( i
  , store { caseBranches = newBranches }
  )
...
registerIfElseBranch region store =
  let
    ( i, newBranches ) = register region $ ifElseBranches store
  in
  ( i
  , store { ifElseBranches = newBranches }
  )
...
registerDeclaration region store =
  let
    ( i, newDeclarations ) = register region $ declarations store
  in
  ( i
  , store { declarations = newDeclarations }
  )
...
emptyStore =
  let
    emptyTypeStore = AnnotationTypeStore { regions = [], counter = 0 }
  in
  AnnotationStore
